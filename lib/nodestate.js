  ...
  NodeState = (function() {
    ...
    function NodeState(config) {
      ...
      this.goto = function(state_name, data) {
        var callback, event_name, previous_state_name, transition, transitions, _ref2, _ref3;
        ...
        transitions = _this.config.transitions;
        if (transitions[previous_state_name] && transitions[previous_state_name][state_name]) {
          transition = transitions[previous_state_name][state_name];
        } else if (transitions['*'] && transitions['*'][state_name]) {
          transition = transitions['*'][state_name];
        } else if (transitions[previous_state_name] && transitions[previous_state_name]['*']) {
